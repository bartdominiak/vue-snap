...
### Removed
- UMD and IIFE builds; `unpkg` field and CJS (`require`) export condition. Package is now ESM-only — use `import` (bundler or Node ESM) instead of `require()` or a `<script>` CDN tag.

## [1.4.0] - 2026-07-09

### Added
- `hideArrows` prop to hide both navigation arrows entirely

## [1.3.0] - 2026-07-09

### Added
- Bundled TypeScript declarations (`.d.ts`) and a `vue-snap/style.css` export

### Changed
- Publish only `dist` via a `files` allowlist, roughly halving package size
