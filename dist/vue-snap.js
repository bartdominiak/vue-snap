var VueSnap = function(exports, vue) {
  ...
  const _sfc_main$1 = /* @__PURE__ */ Object.assign({
    ...
    setup(__props, { emit: __emit }) {
      const emit = __emit;
      const vsWrapper = vue.ref(null);
      const boundLeft = vue.ref(true);
      const boundRight = vue.ref(false);
      const slidesWidth = vue.ref([]);
      const wrapperScrollWidth = vue.ref(0);
      const wrapperVisibleWidth = vue.ref(0);
      const currentPage = vue.ref(0);
      const previousPage = vue.ref(0);
      const currentPos = vue.ref(0);
      ...
      vue.watch(currentPage, (newPage, oldPage) => {
        if (newPage !== oldPage) {
          emit("page", { currentPage: newPage, previousPage: oldPage });
        }
      });
      vue.watch(boundLeft, (state) => {
        if (state) {
          emit("bound-left", true);
        }
      });
      vue.watch(boundRight, (state) => {
        if (state) {
      ...
      return (_ctx, _cache) => {
        return vue.openBlock(), vue.createElementBlock("div", _hoisted_1, [
          ...
          vue.renderSlot(_ctx.$slots, "arrows", {
            changeSlide,
            ...
            vue.withDirectives(vue.createElementVNode("button", {
              ...
              [vue.vShow, __props.hideArrowsOnBound ? !boundLeft.value : true]
            ]),
            vue.withDirectives(vue.createElementVNode("button", {
              type: "button",
              ...
              [vue.vShow, __props.hideArrowsOnBound ? !boundRight.value : true]
            ])
          ])
        ]);
      };
